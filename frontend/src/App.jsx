  ...
  if (authLoading) {
    return (
      <div className="h-screen w-screen bg-black flex items-center justify-center text-white">
        <span className="text-xs font-tech font-bold uppercase tracking-wider text-zinc-400">Loading Environment...</span>
      </div>
    );
  }

  // If no session exists, block app access and display Login screen
  if (!session) {
    return <Login onLoginSuccess={(newSession) => setSession(newSession)} />;
  }

  // Once authenticated, show main dashboard:
  return (
    <div className="h-screen w-screen flex flex-col overflow-hidden bg-black text-zinc-100 font-outfit font-medium antialiased selection:bg-zinc-700">
      ...
      <main className="flex-1 w-full overflow-hidden relative bg-black">
        {activeMode === 'architect' ? (
          <ArchitectView data={mockArchitectData} session={session} />
        ) : (
          <BrainView data={mockBrainData} session={session} />
        )}
      </main>
    </div>
  );
